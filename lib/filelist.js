...
filelist.getAudioFiles = function(dirPath, depth, cb) {
	...
	fs.readdir(dirPath, function(err, files) {
		...
		async.map(itemList,
			...
			function(err, results) {
				...
				for (var i = 0; i < results.length; i++) {
					// only aduio files and directory can be recorded.                              
					if (path.extname(itemList[i].filePath) == '.mp3' || path.extname(itemList[i].filePath) == '.wav') {
						items.push({
							name: itemList[i].file,
							path: itemList[i].filePath,
							type: results[i].stats.isDirectory(),
					...
					if (results[i].stats.isDirectory() == true) {
						queuedItems.push(itemList[i]);
					}
				}
				//console.log('queuedItems: ' + queuedItems);
				async.map(queuedItems, function(obj, cb) {
					filelist.getAudioFiles(obj.filePath, depth - 1, cb);
					return;
				}, function(err, results) {
					for (var i = 0; i < results.length; i++) {
						//console.log(results[i]);
						Array.prototype.push.apply(items, results[i]);
					}

...
playlist.deletePlaylist = function(req, res) {
	...
	if(typeof plName == 'undefined') {
		res.statusCode = 500;
		return res.json({status: 'error', message:'no playlist name to delete'});
	}
...
playlist.createPlaylist = function(req, res) {
	...
	if(typeof newPlName == 'undefined') {
		res.statusCode = 500;
		return res.json({status: 'error', message:'no playlist name'});
	}

	playlist.db[newPlName] = {name: newPlName, songs:[], default:0};
	savePlaylist();
	res.json({status: 'done'});
};
...
playlist.addSongsToPlaylist = function(req, res) {
	...
	if(typeof plName == 'undefined') {
		res.statusCode = 500;
		return res.json({status: 'error', message:'no playlist name to add'});
	}
